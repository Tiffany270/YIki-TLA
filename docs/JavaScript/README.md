...
```js
  ...
  public static ConvertPinyin(ChineseCharacter) {
    ...
    const ChineseCharacterLength = ChineseCharacter.length;
    let phoneticTranscription = '';
    const reg = new RegExp('[a-zA-Z0-9\- ]');
    for (let i = 0; i < ChineseCharacterLength; i++) {
      const val = ChineseCharacter.substr(i, 1);
      const name = this.arraySearch(val, phoneticTranscriptionObj);
      if (reg.test(val)) {
...
// 在对象中搜索
public static arraySearch(litter, phoneticTranscriptionObj) {
  for (const name in phoneticTranscriptionObj) {
...
// 首字母大写
public static ucfirst(initial) {
  if (initial.length > 0) {
    const first = initial.substr(0, 1).toUpperCase();
    const spare = initial.substr(1, initial.length);
    // 返回首字母大写的全拼
    return first + spare;
    // 只返回首字母
    // return first;
...
```js
 // 组件搜索框
  ...
  searchitemList = [];
  searchstoreList = [];
  isgetStorelist = false;
  pinYinList = [];
  nodata = false;
  searchStoreFunction() {
    this.searchStoreIsFocus = true;
  }
  // 转拼音
  handleList() {
    this.searchstoreList = this.siteConfig.storeList;
    for (let i = 0; i < this.searchstoreList.length; i++) {
      const item = Tool.ConvertPinyin(this.searchstoreList[i].title);
      this.pinYinList.push(item);
    }
  ...
  onSearchChange(searchValue: string) {
    if (searchValue === '') {
      this.componentObj.Title = this.componentObj.Title.split(' ')[0];
    }
    if (!this.isgetStorelist) {// 只执行一次
      this.handleList();
    }
    if (!searchValue || searchValue === '') {
      ...
      this.searchitemList = this.searchFilter(searchValue);
  ...
  searchFilter(searchValue) {
    ...
    const mathes2 = item => (
      item &&
      item.indexOf(searchValue.charAt(0).toUpperCase() + searchValue.slice(1)) > -1
    );
    for (let i = 0; i < this.searchstoreList.length; i++) {
      const p = this.searchstoreList[i];
      const t = this.pinYinList[i];
      if (matches(p) || mathes2(t)) {
        res.push(p);
      }
    }
  ...
  selectSearchitem(item: { title: string; }) {
...
### sentry相关
- [参考](https://mp.weixin.qq.com/s/5h5_q6lmbqUy1x__R9C2WA)
- 埋点
- 基于事件去调用后端 api 收集的
-  ssr 框架应该都是基于 node 的吧，node 异步以及事件机制天生对高并发有优势

### 高并发
- 本来是后端的活，但是前端可以控制减少请求，promise, 防抖+节流，await，静态化 cdn ？限流请求合并？扩容加负载均衡
-  node 服务和 react-ssr 渲染，我们这块的方案是高并发时候就直接吐回 html 模板，不进行服务端渲染了，也就是由 SSR 改为 CSR ，渲染这部分交由客户端去做，这样可以节省服务端很多压力
- 做api的拆解
- 但是ssr+node渲染速度堪忧，应该没什么人用吧
